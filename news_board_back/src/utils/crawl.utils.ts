...
import { ENews_Category, NewsCategory, News_Category } from "../configs/category.config"
...
export async function getNews(NewsSection: ENews_Category): Promise<any[]> {
    let newsBox = []
    ...
    list.each((index, elem) => {
        ...
        if (!Object.values(newsObj).filter(value => !value).length) newsBox.push(newsObj)
    })
